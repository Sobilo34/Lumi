"""B4 end-to-end Teacher Report flow tests (headless pygame)."""
...
def test_b4_open_teacher_report_builds_live_report(headless_engine: GameEngine) -> None:
...
def test_b4_recommended_practice_navigates_to_bd_practice(headless_engine: GameEngine) -> None:
    engine = headless_engine
    engine.set_screen("teacher_report")

    action = _click_named_hitbox(engine, "Recommended practice")
    assert action == "practice_recommendation"
    engine._handle_action(action)

    assert engine.state.current_screen_id == "bd_practice"
    assert engine.state.bd_practice_target == "B"


@pytest.mark.parametrize("action", ["back", "home"])
def test_b4_back_and_home_return_to_main_menu(headless_engine: GameEngine, action: str) -> None:
    engine = headless_engine
    engine.set_screen("teacher_report")

    if action == "back":
        clicked = _click_named_hitbox(engine, "Back")
    else:
...
def test_b4_word_garden_recommendation_when_bd_not_weak_enough(tmp_path: Path) -> None:
    profile = {
        "child_name": "Player 1",
        "total_stars": 4,
        "attempts": 5,
    ...
    screen = pygame.display.set_mode((1280, 720))
    engine = GameEngine(screen)
    engine.learner = LearnerModel(profile_path=tmp_path / "player_1.json", profile_data=profile)
    engine.set_screen("teacher_report")
...
def test_teacher_report_hitboxes_match_expected_actions() -> None:
    ...
    assert actions["Recommended practice"] == "practice_recommendation"


def test_b4_saved_session_json_round_trip(tmp_path: Path) -> None:
    ...
    assert loaded["needs_practice"] == "Letters B and D"
